...
def _extract_tls_details(hostname: str, port: int) -> dict[str, Any]:
    ...
    subject = dict(cert.get("subject", [])[0]) if cert.get("subject") else {} # type: ignore
    issuer = dict(cert.get("issuer", [])[0]) if cert.get("issuer") else {} # type: ignore

    return {
        "subject": subject.get("commonName", ""), # type: ignore
        "issuer": issuer.get("commonName", ""), # type: ignore
        "expires_at": cert.get("notAfter", ""), # type: ignore
...
def inspect_target(target: str) -> dict[str, Any]:
    ...
    try:
        ip_addresses = _resolve_host(final_host) # type: ignore
    except socket.gaierror as exc:
        raise ValueError("No se ha podido resolver el dominio indicado.") from exc

    parser = DocumentStatsParser(base_host=final_host) # type: ignore
    parser.feed(response.text[:250000])

    tls_details = None
    if response.url.startswith("https://"):
        try:
            tls_details = _extract_tls_details(final_host, final_port) # type: ignore
        except OSError:
